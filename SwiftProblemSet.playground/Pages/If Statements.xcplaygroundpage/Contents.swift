...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    if (bugs < 10) && music && (levels > 5) {
        ...
        if (bugs >= 10) {
            print("Needs to fix at least \(bugs - 9) bug(s).")
        }
        ...
        if (levels <= 5) {
            print("Needs \(6 - levels) more level(s).")
        }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    var statement = "\(name) is "
    if !bike && !run {
        statement += "not ready for both events."
    } else if !bike {
        statement += "not ready for biking."
    } else if !run {
        statement += "not ready for running."
    } else {
        statement += "ready."
    }
